...
export class CustomersListPage {
  ...
  async assertCustomerRowIsNotPresent(firstName, lastName, postalCode) {
    await expect(this.tableRows.last()).not.toHaveText(firstName);
    await expect(this.tableRows.last()).not.toHaveText(lastName);
    await expect(this.tableRows.last()).not.toHaveText(postalCode);
  }
